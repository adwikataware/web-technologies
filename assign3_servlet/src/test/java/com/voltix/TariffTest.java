...
class TariffTest {
    ...
    void lowReadingHasOneSlab() {
        ...
        assertEquals("0 - 50 units", slabs.get(0).label());
        assertEquals(new BigDecimal("30"), slabs.get(0).units());
        assertEquals(new BigDecimal("105.00"), slabs.get(0).amount());
    }
    ...
    void highReadingSpansEverySlab() {
        ...
        assertEquals(new BigDecimal("50"), slabs.get(0).units());
        assertEquals(new BigDecimal("100"), slabs.get(1).units());
        assertEquals(new BigDecimal("100"), slabs.get(2).units());
        assertEquals(new BigDecimal("90"), slabs.get(3).units());
        assertEquals("Above 250 units", slabs.get(3).label());
